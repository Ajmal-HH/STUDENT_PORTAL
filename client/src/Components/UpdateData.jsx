...
const UpdateData = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:5001/fetchdata/${id}`)
            .then((response) => {
                const { name, email, age } = response.data[0]
                setName(name)
                setEmail(email)
                setAge(age)
            })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(`http://localhost:5001/update/${id}`, { name, email, age })
                .then(() => {
    ...
    return (
        <Container className="mt-5">
            <Row className="justify-content-md-center">
                <Col md={6}>
                    <Card>
                        <Card.Header as="h5" className="text-center">UPDATE STUDENT DATA</Card.Header>
                        <Card.Body>
